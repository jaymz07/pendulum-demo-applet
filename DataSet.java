...
public class DataSet
{
	...
	public double interpolate(double xPoint)
	{
	  ...
	  {
	    if(data.get(i).x<=xPoint&&data.get(i+1).x>=xPoint) {
	      ind=i;
	      break;
	    }
	  }
	  double m=(data.get(ind+1).y-data.get(ind).y)/(data.get(ind+1).x-data.get(ind).x);
	  return data.get(ind).y+m*(xPoint-data.get(ind).x);
	}
